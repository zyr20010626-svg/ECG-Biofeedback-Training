#!/usr/bin/env python3
"""
ECG Biofeedback Training — PsychoPy Native Version
内感受视觉训练干预方案 - BMD101 心电传感器版本

基于 PsychoPy 原生 API（visual, event, gui），可直接在 PsychoPy Coder 中运行。

实验流程：
- 3 个训练组块，每块包含反馈/无反馈两个阶段
- 每阶段 24 试次，共 144 试次
- 反馈阶段：R 波后立即呈现动态心脏图标 + 红色脉冲边框
...
HEART_APPEAR_DELAY_MS = 0         # delay before red signal appears (heart is immediate); 实验说明: 0ms 立即呈现
N_MIN = 5
N_MAX = 10
...
# ================================================================
#  R-Peak Detector  (identical to PyQt5 version)
# ================================================================
class RPeakDetector:
...
# ================================================================
#  Experiment State Machine  (adapted from PyQt5 version)
# ================================================================
class ExperimentController:
    ...
    def start_experiment(self, now):
        ...
        self.instruction_text = "正在连接心电传感器..."
        self.state = 'starting'
    ...
    def _on_r_peak_experiment(self, now):
        if self.state == 'trial_active':
            ...
            if self.trial_r_wave_count == self.current_target_n:
                ...
                # Red signal scheduled via update() — 200ms after target
                # (handled in the update() method above)

            # In feedback phase: heart icon beats on every R-wave (handled via heart_beat_time)
            # No separate signal needed — the draw loop reads heart_beat_time
    ...
    def _start_block(self, now):
        ...
        self.phase_info_text = f"组块 {self.current_block}/{BLOCKS_PER_SESSION} — 反馈阶段"
    ...
    def _start_phase(self, now):
        ...
        if self.current_phase == 'feedback':
            ...
            self.phase_info_text = f"组块 {self.current_block}/{BLOCKS_PER_SESSION} — 无反馈阶段"
...
def show_config_dialog():
    ...
    dlg = gui.Dlg(title="ECG 生物反馈训练 — 实验设置")
    dlg.addField("被试编号:", "S001")
    dlg.addField("训练次数:", "1")
...
class PsychopyExperiment:
    ...
    def _create_stimuli(self):
        ...
        # We'll draw 3 nested rectangles with varying opacity each frame.
        # We create the RectStim once and adjust each frame.
    ...
    def _draw(self, now):
        ...
        if ctrl.state in ('trial_active', 'trial_feedback'):
            ...
            self.stim['status'].text = "实验完成 — 按 Esc 退出"
        else:
