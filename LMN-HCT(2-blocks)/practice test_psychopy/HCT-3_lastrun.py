...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\李大黄\\Desktop\\psychopy\\HCT-3_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    introducion_textbox = visual.TextBox2(
         win, text='心跳计数任务\n\n实验中，会让你试着去感受你的心跳，不是主动地感觉(触摸)你的手腕或脖子的脉动。你可能有感觉，也可能根本感觉不到，或者偶尔会有一些感觉。\n\n当听到第一个信号时，请将注意放到你的心跳上；5s后将会出现第二个信号，请你开始数你感觉到的心跳。当听到第三个信号时，请停止数心跳并输入你数的数量以及对你的自信心进行打分。\n\n对这项任务，只计算你真正感觉到的心跳，不要试图猜测你的心率。非常强调，只统计你感觉到的心跳。即使你报告没有感觉到心跳。\n\n如果对实验有何问题，及时询问主试。如果已经理解，点击键盘任意按键进入实验。', placeholder='Type here...', font='Microsoft YaHei',
    ...
    for thisHeartbeat_Loop in heartbeat_Loop:
        ...
        while continueRoutine and routineTimer.getTime() < 15.0:
            ...
            if Rest_text.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > Rest_text.tStartRefresh + 15-frameTolerance:
                    # keep track of stop time/frame for later
    ...
    # completed 6.0 repeats of 'heartbeat_Loop'
